...
import { MeshoptDecoder, GLTFLoader } from 'three-stdlib';

try { (useGLTF as any).setMeshoptDecoder?.(MeshoptDecoder); } catch (e) {}
...
function PreloadGLB(props: { url: string; onBvhDone: (url: string) => void }) {
...
function PreloadTex(props: { url: string }) {
...
function PreloadOne(props: { url: string; onBvhDone: (url: string) => void }) {
  ...
  if (e === 'glb' || e === 'gltf') {
    return <PreloadGLB url={props.url} onBvhDone={props.onBvhDone} />;
  }

  if (e === 'jpg' || e === 'jpeg' || e === 'png' || e === 'webp') {
    return <PreloadTex url={props.url} />;
  }
