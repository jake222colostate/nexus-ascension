...
export function GameHUD(props: {
  currentWorld: 'Hub' | 'Fantasy' | 'Skybase';
  onSwitchWorld: (target: 'Hub' | 'Fantasy' | 'Skybase') => void;
  manaText: string;
        ...
        <View style={hudStyles.topWrap}>
          ...
          <View style={hudStyles.row}>
            ...
            <View style={hudStyles.rightStack}>
              {props.statRight.map((s) => <StatPill key={s} text={s} />)}
            </View>
          </View>
        </View>
        ...
        <View style={{ position: 'absolute', left: 10, right: 10, bottom: insets.bottom + 10, backgroundColor: '#101826', borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255,255,255,0.2)', maxHeight: '75%' }}>
          ...
          <ScrollView contentContainerStyle={{ padding: 12, gap: 8 }}>
            {upgrades.map((up) => {
              ...
              return (
                <View key={up.id} style={{ borderRadius: 14, padding: 12, backgroundColor: 'rgba(255,255,255,0.06)', borderWidth: 1, borderColor: 'rgba(255,255,255,0.18)' }}>
